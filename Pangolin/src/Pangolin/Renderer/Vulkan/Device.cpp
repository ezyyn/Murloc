...
namespace PG {
	...
	LogicalDevice::LogicalDevice()
	{
		...
		PG_VK_ASSERT(vkCreateDevice(m_PhysicalDevice.GetNative(), &createInfo, nullptr, &m_NativeDevice));
		...
		PG_VK_ASSERT(vkCreateCommandPool(m_NativeDevice, &poolInfo, nullptr, &m_CommandPool));
	...
	PhysicalDevice::PhysicalDevice()
	{
		...
		PG_VK_ASSERT(vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr));
		PG_CORE_ASSERT(deviceCount, "Failed to find GPUs with Vulkan support!");

		std::vector<VkPhysicalDevice> devices(deviceCount);
		PG_VK_ASSERT(vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data()));
		...
		PG_CORE_ASSERT(m_NativePhysicalDevice, "Failed to find suitable GPU!");
	...
	void PhysicalDevice::PopulateSwapchainSupportDetails()
	{
		...
		PG_VK_ASSERT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_NativePhysicalDevice, surface, &capabilities));

		// Query for swap chain available formats
		uint32_t formatCount;
		PG_VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(m_NativePhysicalDevice, surface, &formatCount, nullptr));

		if (formatCount != 0) {
			formats.resize(formatCount);
			PG_VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(m_NativePhysicalDevice, surface, &formatCount, formats.data()));
		}
		// Query for swap chain available present modes
		uint32_t presentModeCount;
		PG_VK_ASSERT(vkGetPhysicalDeviceSurfacePresentModesKHR(m_NativePhysicalDevice, surface, &presentModeCount, nullptr));

		if (presentModeCount != 0) {
			presentModes.resize(presentModeCount);
			PG_VK_ASSERT(vkGetPhysicalDeviceSurfacePresentModesKHR(m_NativePhysicalDevice, surface, &presentModeCount, presentModes.data()));
		}
	...
	bool PhysicalDevice::IsDeviceSuitable(VkPhysicalDevice device)
	{
		...
		for (const auto& queueFamily : queueFamilies) {
			...
			PG_VK_ASSERT(vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surface, &presentSupport));
