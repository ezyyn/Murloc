...
namespace PG {
		...
		: m_Size(size)
	{
		...
		PG_VK_ASSERT(vkCreateBuffer(device, &bufferInfo, nullptr, &m_Buffer));
		...
		PG_VK_ASSERT(vkAllocateMemory(device, &allocInfo, nullptr, &m_Memory));

		PG_VK_ASSERT(vkBindBufferMemory(device, m_Buffer, m_Memory, 0));
	...
	void VulkanBuffer::SetData(const void* data)
	{
		...
		void* dest;
		PG_VK_ASSERT(vkMapMemory(device, m_Memory, 0, m_Size, 0, &dest));
