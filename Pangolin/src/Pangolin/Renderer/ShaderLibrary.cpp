...
namespace PG {
	...
	bool ShaderLibrary::CheckUpToDate(ShaderData& shaderData)
	{
		...
		if (in)
		{
			...
			{
				PG_CORE_ERROR("Could not read from file '{0}'", m_ShaderLibraryFilePath);
			}
		}
		else
		{
			PG_CORE_ERROR("Could not open file '{0}'", m_ShaderLibraryFilePath);
		}
		// hash string
	...
	{
		PG_CORE_WARN("GENERATING SHADER: {0}", shaderData.FilePath);
	...
	void ShaderLibrary::Reflect(ShaderStage stage, const std::vector<uint32_t> compiled, const std::string& filepath)
	{
		spirv_cross::Compiler compiler(compiled);
		spirv_cross::ShaderResources resources = compiler.get_shader_resources();

		PG_CORE_TRACE("GLSLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), filepath);
		PG_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		PG_CORE_TRACE("    {0} resources", resources.sampled_images.size());
		...
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			PG_CORE_TRACE("  {0}", resource.name);
			PG_CORE_TRACE("    Size = {0}", bufferSize);
			PG_CORE_TRACE("    Binding = {0}", binding);
			PG_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
