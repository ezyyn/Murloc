...
namespace PG {
	...
	static ScriptData* m_RenderData{ nullptr };

	void ScriptEngine::Init()
	{
		m_RenderData = new ScriptData;

		mono_set_assemblies_path("mono/lib");

		MonoDomain* rootDomain = mono_jit_init("MurlocJITRuntime");
		PG_CORE_ASSERT(rootDomain);

		// Store the root domain pointer
		m_RenderData->RootDomain = rootDomain;

		// Create an App Domain
		m_RenderData->AppDomain = mono_domain_create_appdomain("MyAppDomain", nullptr);
		mono_domain_set(m_RenderData->AppDomain, true);

		m_RenderData->CoreAssembly = LoadCSharpAssembly("assets/scripts/Murloc-ScriptCore.dll");
		Utils::PrintAssemblyTypes(m_RenderData->CoreAssembly);

		// Gets image of the assembly
		MonoImage* image = mono_assembly_get_image(m_RenderData->CoreAssembly);
		// Get a reference to the class we want to instantiate
		MonoClass* klass = mono_class_from_name(image, "Murloc", "Main");
		PG_CORE_ASSERT(klass);
		// Allocates new object
		MonoObject* klassInstance = mono_object_new(m_RenderData->AppDomain, klass);
		PG_CORE_ASSERT(klassInstance)
		...
		{
			MonoString* monoString = mono_string_new(m_RenderData->AppDomain, "Hello World from C++");
			void* params[1]{
				monoString
			};
	...
	void ScriptEngine::Shutdown()
	{
		...
		mono_jit_cleanup(m_RenderData->RootDomain);
		//mono_domain_unload(s_Data->AppDomain);
		m_RenderData->AppDomain = nullptr;
		m_RenderData->RootDomain = nullptr;
