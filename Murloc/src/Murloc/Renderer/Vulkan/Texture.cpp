...
namespace H {
	static void CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
		...
		auto device = Murloc::VulkanContext::GetLogicalDevice()->GetNative();
		...
		allocInfo.memoryTypeIndex = Murloc::Utils::FindMemoryType(memRequirements.memoryTypeBits, properties);
...
namespace Murloc {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			stbi_uc* pixels = stbi_load(/*"assets/textures/texture.jpg"*/m_FilePath.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
			VkDeviceSize imageSize = texWidth * texHeight * 4;

			MUR_ASSERT(imageSize);

			m_Width = texWidth;
			m_Height = texHeight;

			stagingBuffer = new VulkanBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		...
		ScopeCommandPool tempPool;
		{
			...
			{
				H::RecordTransititonImageLayout(m_TextureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, commandBuffer);
			}
	...
	void Texture::CreateImage()
	{
		...
		// Creating image 
		{
			...
			MUR_VK_ASSERT(vkCreateImage(device, &imageInfo, nullptr, &m_TextureImage));
			...
			MUR_VK_ASSERT(vkAllocateMemory(device, &allocInfo, nullptr, &m_TextureImageMemory));

			MUR_VK_ASSERT(vkBindImageMemory(device, m_TextureImage, m_TextureImageMemory, 0));
		}
	}
	...
	void Texture::CreateTextureImageView()
	{
		...
		MUR_VK_ASSERT(vkCreateImageView(device, &viewInfo, nullptr, &m_TextureImageView));
	}
	...
	void Texture::CreateTextureSampler()
	{
		...
		MUR_VK_ASSERT(vkCreateSampler(device->GetNative(), &samplerInfo, nullptr, &m_TextureSampler));
	}

	void Texture::CreateDescriptorSets()
	{
		auto device = VulkanContext::GetLogicalDevice()->GetNative();
		auto framesInFlight = Renderer::GetSettings().FramesInFlight;
		{
			...
			MUR_VK_ASSERT(vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &m_DescriptorSetLayout))
		}
		// Descriptor pool
		// Descriptor pool
		// Descriptor pool
